...
sample_csv= pd.read_csv('test_data.csv', header=None)
golden_csv = pd.read_csv('golden_data.csv')
...
for i in range(0, 100):
    test_dict_small[mcn_arr_test[i]] = train_arr[i]

golden_dict_small = {}
# golden_dict.setdefault('dummy',[])
for i in range(0, 1000):
    golden_dict_small[mcn_arr_golden[i][0]] = golden_flat_list[i]
...
for k in range(0, 4):
    test_data_split_features_k = []   
    for i in range(0, len(test_data)):
        z= test_data[i].split(' ')
        z[k:-(k+1)] = []
        z = ' '.join(z)
        test_data_split_features_k.append(z)

    test_data_split_features.append(test_data_split_features_k)
print(test_data_split_features)

choices = golden_dict_small.values()
score= 0
found_flag=0
not_found={}
accuracy_list=[]

for k in range(0, len(test_data_split_features)):
    not_found_iteration = []    
    for i in range(0, len(test_data_split_features[k])-1):
        
        found_flag=0
        chlist=process.extract(test_data_split_features[k][i], choices, limit=2)
    #     print("{},   --test--{}".format(test_data[i], mcn_arr_test[i]))
        
        for mcn,value in golden_dict_small.items():
            for j in range(0,2):
                if value == chlist[j][0]:
    ...
    accuracy = 100* score/len(test_data_split_features[k])
